...
string dataTypeToString(int type) {
    ...
    if (type == 1) return "AtvirkÅ¡Äiai surikiuoti";
...
int main() {
    ...
    cout << "Vykdomi skaiÄiavimai..." << endl;

    for (int n : sizes) {
        for (int dataType = 0; dataType < 3; ++dataType) {
            cout << "SkaiÄiuoju n = " << n << ", tipas = " << dataTypeToString(dataType) << "..." << endl;
    ...
    cout << "\nSkaiÄiavimai baigti! Rezultatai:\n" << endl;

    cout << left << setw(15) << "Algoritmas"
         << setw(10) << "Dydis"
         << setw(25) << "DuomenÅ³ tipas"
         << setw(20) << "Vid. laikas (Âµs)"
         << setw(20) << "Vid. palyginimÅ³"
         << setw(20) << "Vid. sukeitimÅ³" << "\n";
    ...
    cout << "\nPrograma baigÄ— darbÄ… sÄ—kmingai!" << endl;
    system("pause");
    return 0;
}
